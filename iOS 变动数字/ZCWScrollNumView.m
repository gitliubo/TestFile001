...
- (void)setDigitAndCommit:(NSUInteger)aDigit {
    ...
    self.label.text = [NSString stringWithFormat:@"%ld", aDigit];
...
- (void)setDigit:(NSUInteger)aDigit from:(NSUInteger)last{
    ...
    NSMutableString *str = [NSMutableString stringWithFormat:@"%ld", last];
    int count = 1;
    if (aDigit > last) {
...
- (void)setRandomScrollDigit:(NSUInteger)aDigit length:(NSUInteger)length{
    NSMutableString *str = [NSMutableString stringWithFormat:@"%ld", self.digit];
...
//    CGSize size= [@"8" sizeWithFont:self.digitFont];
    ...
    CGSize size=  [@"8" boundingRectWithSize:CGSizeMake(CGFLOAT_MAX,CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.digitFont} context:nil].size;
